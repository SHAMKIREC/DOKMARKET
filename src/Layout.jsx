    ...
    <div className="app-shell" style={{ fontFamily: "'Inter', sans-serif", color: "#e2e8f0", minHeight: "100vh" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .app-shell { background-color:#080910; background-image:radial-gradient(circle at 18% 0%,rgba(37,99,235,.13),transparent 34rem),radial-gradient(circle at 82% 4%,rgba(124,58,237,.14),transparent 32rem),radial-gradient(circle at 50% 55%,rgba(67,56,202,.045),transparent 42rem); background-attachment:fixed; }
        .nav-link { color:#cbd5e1; font-size:.9rem; font-weight:500; text-decoration:none; transition:.2s; cursor:pointer; background:none; border:none; padding:7px 4px; border-radius:8px; white-space:nowrap; }
        .nav-link:hover { color:#a5f3fc; } .nav-link.active { color:#c4b5fd; }
        ...
        .platform-pill { display:inline-flex; align-items:center; gap:6px; padding:5px 9px; border-radius:999px; color:#a5f3fc; border:1px solid rgba(103,232,249,.18); background:rgba(8,145,178,.08); font-size:.66rem; font-weight:800; white-space:nowrap; }
      ...
      {!isMarketplace && <nav style={{ position:"fixed",top:0,left:0,width:"100%",zIndex:50,background:"rgba(7,9,16,.96)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(103,232,249,.08)" }}>
        <div className="header-inner" style={{ maxWidth:"1380px",margin:"0 auto",padding:"0 1.25rem",display:"flex",justifyContent:"space-between",alignItems:"center",minHeight:"76px" }}>
          <Link to="/dosudebka" className="brand-logo-link" aria-label="Досудебка — сервис ДокМаркет" onClick={() => setMenuOpen(false)}>
            <img src={logoDosudebka} alt="Досудебка" className="brand-logo-image" />
            <span className="platform-pill"><i className="fa-solid fa-cubes" />сервис ДокМаркет</span>
          </Link>
          <div className="desktop-nav" style={{ alignItems:"center",gap:"clamp(13px,1.5vw,22px)",marginLeft:24 }}>
            <Link to="/" className="nav-link"><i className="fa-solid fa-arrow-left" style={{marginRight:6}} />ДокМаркет</Link>
        ...
        {menuOpen && <div className="mobile-menu" style={{flexDirection:"column",background:"rgba(7,9,16,.99)",borderTop:"1px solid rgba(255,255,255,.07)",padding:"8px 0 12px"}}>
          {[{label:"← ДокМаркет",to:"/",page:"Market"},{label:"Досудебка",to:"/dosudebka",page:"Dosudebka"},{label:"Создать претензию",page:"Generator"},{label:"Как работает",page:"Guide"},{label:"Тарифы",page:"Pricing"},{label:isLawyer?"Бизнес-кабинет":"Личный кабинет",page:isLawyer?"BusinessCabinet":"Dashboard"}].map(item=><Link key={item.label} to={item.to||createPageUrl(item.page)} className="nav-link" onClick={()=>setMenuOpen(false)} style={{display:"block",padding:"14px 24px",fontSize:"1rem",borderBottom:"1px solid rgba(255,255,255,.05)"}}>{item.label}</Link>)}
        </div>}
      </nav>}
      <main>{children}</main>
    </div>
