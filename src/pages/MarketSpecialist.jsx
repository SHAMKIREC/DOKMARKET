...
export default function MarketSpecialist(){
 ...
 if(!specialist)return <MarketFrame><MarketNavigation crumbs={[{label:"ДокМаркет",to:"/market"},{label:"Продавец не найден"}]} backTo="/market"/><div className="market-empty market-glass"><h1 className="market-heading">Продавец не найден</h1></div></MarketFrame>;
 ...
 return <MarketFrame>
  ...
  <div className="market-deal-note market-glass"><strong>Заказ проходит через ДокМаркет</strong><span>Задача и результат остаются в заказе. После подключения платёжного модуля здесь же будет безопасная оплата и расчёт с исполнителем после подтверждения результата.</span></div>
