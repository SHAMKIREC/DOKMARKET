...
const benefits = [
  ...
  ["Своя витрина", "У каждого специалиста единая карточка с товарами, услугами, рейтингом и отзывами."],
...
const flow = [
  ...
  ["4", "Завершайте работу", "После подключения оплаты выплата специалисту будет проходить после подтверждения выполнения заказа покупателем."],
];

export default function ForLawyers() {
  return <main className="dm-seller-page">
    <section className="dm-seller-hero">
      <span className="dm-seller-kicker">СПЕЦИАЛИСТ ДОКМАРКЕТА</span>
      ...
      <div className="dm-seller-actions">
        <Link className="dm-seller-primary" to="/RegisterLawyer">Стать специалистом</Link>
    ...
    <section className="dm-seller-flow">
      <div className="dm-seller-flow-head"><span>КАК ЭТО РАБОТАЕТ</span><h2>Один специалист — одна витрина — все заказы внутри приложения</h2><p>Покупатель видит рейтинг и отзывы, выбирает документ или услугу и не уходит в сторонние чаты для оформления заказа.</p></div>
    ...
    <section className="dm-seller-note">
      <div><strong>Регистрация специалиста бесплатная</strong><p>Отдельной подписки для входа в маркетплейс сейчас нет. CRM, массовая генерация, расширенная автоматизация и другие профессиональные инструменты можно подключать позже как самостоятельные продукты.</p></div>
      <Link to="/RegisterLawyer">Создать кабинет специалиста →</Link>
    </section>
