...
export default function CatalogOfferRouter(){
 ...
 return <MarketFrame>
  ...
  <section className="market-product-layout">
   <div className="market-panel market-glass market-product-main"><div className="market-offer-top"><div style={{display:"flex",gap:7,flexWrap:"wrap"}}><span className="market-badge">{item.providerType==="platform"?"От ДокМаркета":"От продавца"}</span>{item.formats.map(x=><span className="market-badge" key={x}>{x}</span>)}</div></div><h1 className="market-heading">{item.title}</h1><p className="market-lead">{item.description}</p>
