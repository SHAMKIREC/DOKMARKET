...
export default function RegisterLawyer(){
 ...
 async function submit(e){e.preventDefault();if(!validate())return;setBusy(true);try{await registerLawyer({...form,email:form.email.trim().toLowerCase()});navigate("/BusinessCabinet",{replace:true})}catch(error){if(error.message==="EMAIL_CONFIRMATION_REQUIRED")setNotice("Аккаунт продавца создан. Подтвердите email, затем войдите — карточка продавца создастся автоматически.");else setErrors(c=>({...c,form:error.message==="EMAIL_EXISTS"?"Аккаунт с таким email уже существует.":"Не удалось создать аккаунт. Проверьте данные."}))}finally{setBusy(false)}}
 return <AuthShell title="Стать продавцом ДокМаркета" subtitle="Публикуйте документы и услуги. После регистрации заполните карточку продавца и отправьте товары на модерацию.">
  <form onSubmit={submit} noValidate style={{display:"grid",gap:12}}>
   ...
   <Field name="organizationName" label="Бренд или организация, необязательно" placeholder="Юридический кабинет Петрова" value={form.organizationName} error={errors.organizationName} onChange={setValue} hint="Если работаете как частный специалист — оставьте пустым."/>
   ...
   <Consent checked={form.sellerTerms} onChange={v=>setValue("sellerTerms",v)} error={errors.sellerTerms}>Я принимаю правила ДокМаркета для продавцов и специалистов</Consent>
   {notice&&<p role="status" style={{color:"#86efac",margin:0,fontSize:".8rem"}}>{notice}</p>}{errors.form&&<ErrorText>{errors.form}</ErrorText>}
   <button style={{...button,opacity:busy?.7:1}} disabled={busy}>{busy?"Создаём…":"Создать кабинет продавца"}</button>
  </form>
  ...
  <Link to="/Login" style={{color:"#c4b5fd",display:"inline-block",marginTop:18,fontSize:".84rem"}}>Уже зарегистрированы? Войти</Link>
 </AuthShell>
}
