...
export default function RegisterLawyer(){
 ...
 async function submit(e){e.preventDefault();if(!validate())return;setBusy(true);try{await registerLawyer({...form,email:form.email.trim().toLowerCase()});setNotice("Аккаунт создан, заявка специалиста отправлена на модерацию. После одобрения станет доступен кабинет специалиста.");setTimeout(()=>navigate("/market",{replace:true}),900)}catch(error){if(error.message==="EMAIL_CONFIRMATION_REQUIRED")setNotice("Аккаунт создан. Подтвердите email и войдите. После входа отправьте заявку специалиста повторно, если она ещё не появилась в профиле.");else setErrors(c=>({...c,form:error.message==="EMAIL_EXISTS"?"Аккаунт с таким email уже существует.":"Не удалось создать аккаунт. Проверьте данные."}))}finally{setBusy(false)}}
 return <AuthShell title="Стать специалистом" subtitle="Создайте аккаунт и отправьте заявку. После модерации вы сможете публиковать документы, услуги и получать заказы внутри ДокМаркета.">
  <form onSubmit={submit} noValidate style={{display:"grid",gap:11}}>
   ...
   <Field name="email" label="Email" type="email" placeholder="specialist@example.ru" value={form.email} error={errors.email} onChange={setValue}/>
   ...
   <Field name="organizationName" label="Название бюро или бренда, необязательно" placeholder="Например: Бюро документов" value={form.organizationName} error={errors.organizationName} onChange={setValue} hint="Если работаете от своего имени — оставьте пустым."/>
   ...
   <Consent checked={form.sellerTerms} onChange={v=>setValue("sellerTerms",v)} error={errors.sellerTerms}>Я принимаю правила ДокМаркета для специалистов</Consent>
   {notice&&<p role="status" style={{color:"#86efac",margin:0,fontSize:".78rem",lineHeight:1.5}}>{notice}</p>}{errors.form&&<ErrorText>{errors.form}</ErrorText>}
   <button style={{...button,opacity:busy?.7:1}} disabled={busy}>{busy?"Отправляем…":"Создать аккаунт и отправить заявку"}</button>
  </form>
  <section className="dm-seller-steps"><span><b>1</b>Создайте аккаунт</span><span><b>2</b>Отправьте заявку на модерацию</span><span><b>3</b>После одобрения заполните витрину</span><span><b>4</b>Добавляйте документы и услуги</span></section>
