...
export default function Generator() {
  ...
  async function restoreOwnerRoom() {
    ...
    try {
      ...
      const restored = {
        ...
        selectedLegalOptions: owner?.selectedLegalOptions?.length ? owner.selectedLegalOptions : room.commonData?.selectedLegalOptions || [],
  ...
  async function initJoiner(roomId) {
    ...
    try {
      ...
      if (!room || !member) {
        setError("Комната совместной претензии не найдена.");
        return;
      }
      ...
      const newData = {
        ...
        selectedLegalOptions: member.selectedLegalOptions || [],
      };
      ...
      const restoredData = parsedDraft?.claimData
        ? { ...newData, ...parsedDraft.claimData, roomId, collectiveParticipantId: participantToken, isJoiner: true }
        : newData;
  ...
  async function handleFinalize() {
    try {
      const room = await getRoom(claimData.roomId);
      if (!room) throw new Error("ROOM_NOT_FOUND");
      const completedMembers = getCompletedParticipants(room);
      if (completedMembers.length < 2) {
        setError("Для совместной претензии нужны минимум 2 заполненных участника.");
        return;
      }
      ...
      setClaimData(prev => ({
        ...prev,
        employer: room.employer || prev.employer,
        collectiveMembers: completedMembers,
        collectiveFinalized: true,
        completedParticipantsCount: completedMembers.length,
        workers: completedMembers.map(memberToWorker),
  ...
  if (joinLoading) {
    return <div className="min-h-screen pt-24 flex items-center justify-center"><div className="w-8 h-8 border-4 border-slate-700 border-t-purple-500 rounded-full animate-spin"></div></div>;
  }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isJoiner && (
          <div style={{ background: "rgba(139,92,246,0.1)", border: "1px solid rgba(139,92,246,0.3)", borderRadius: 12, padding: "12px 18px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ color: "#d1d5db", fontSize: "0.88rem" }}><b style={{ color: "#a78bfa" }}>Вы участник совместной претензии</b> — заполните только свои личные данные</span>
          </div>
        )}
